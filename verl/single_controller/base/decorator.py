...
def _concat_data_proto_or_future(output: List):
    ...
    if isinstance(o, DataProto):
        ...
        dataproto = [o[0] for o in output]
        if isinstance(dataproto[0], DataProto):
            concat_dataproto = DataProto.concat(dataproto)
        elif isinstance(dataproto[0], ray.ObjectRef):
            concat_dataproto = DataProtoFuture.concat(dataproto)
        else:
            raise NotImplementedError
        id2pg_loss = [o[1] for o in output]
        if isinstance(id2pg_loss[0], defaultdict):
            concat_id2pg_loss = defaultdict(list)
            for d in id2pg_loss:
                for key, value in d.items():
                    concat_id2pg_loss[key].extend(value)
        else:
            raise NotImplementedError
        return concat_dataproto, concat_id2pg_loss
    else:
        raise NotImplementedError
...
def collect_dp_compute_data_proto(worker_group, output):
    ...
    for o in output:
        assert isinstance(o, (DataProto, ray.ObjectRef)) or isinstance(o, tuple), f"expecting {o} to be DataProto or Tuple, but got {type(o)}"
