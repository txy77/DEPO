...
class DataProto:
    ...
    def select_via_index(self, indices) -> 'DataProto':
        """Select a subset of the DataProto via indices """
        # select the batch
        sub_batch = self.batch[indices]
        # select the non_tensor_batch
        sub_non_tensor_batch = {key: val[indices] for key, val in self.non_tensor_batch.items()}
